    ...
    <div className="min-h-screen bg-slate-950 text-white">
      ...
      <section className="mx-auto max-w-4xl px-8 pt-24 pb-16 text-center">
        ...
        <h1 className="text-5xl font-bold leading-tight tracking-tight md:text-6xl">
          Живые голосования
          <br />
          <span className="text-indigo-400">для ваших мероприятий</span>
        </h1>
        ...
        <div className="mt-10 flex flex-col sm:flex-row gap-4 justify-center">
          <Link
            href="/auth/register"
            className="rounded-xl bg-indigo-600 hover:bg-indigo-500 px-8 py-3.5 text-base font-semibold transition-colors"
          >
            Попробовать бесплатно
          </Link>
          <Link
            href="/auth/login"
            className="rounded-xl border border-slate-700 hover:border-slate-600 bg-slate-900 hover:bg-slate-800 px-8 py-3.5 text-base font-semibold transition-colors"
          >
            Войти в аккаунт
          </Link>
        </div>
      ...
      <section className="mx-auto max-w-5xl px-8 pb-24">
        <div className="rounded-2xl border border-slate-800 bg-slate-900 overflow-hidden">
          ...
          <div className="flex items-center gap-2 px-4 py-3 border-b border-slate-800 bg-slate-900/80">
            ...
            <span className="h-3 w-3 rounded-full bg-slate-700" />
            <span className="h-3 w-3 rounded-full bg-slate-700" />
            <span className="mx-auto text-xs text-slate-600 font-mono">livepoll.ru/display/ABC123</span>
          </div>
          {/* Fake display screen */}
          <div className="p-8 flex gap-6 min-h-[280px]">
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-6">
                <span className="flex items-center gap-1.5 rounded-full bg-green-500/10 border border-green-500/20 px-3 py-1 text-xs text-green-400 font-medium">
                  <span className="h-1.5 w-1.5 rounded-full bg-green-400 animate-pulse" />
                  LIVE
                </span>
                <span className="text-xs text-slate-500">47 голосов</span>
              </div>
              <h3 className="text-xl font-bold text-white mb-6">Насколько вы довольны мероприятием?</h3>
              <div className="flex flex-col gap-2">
                {[
                  ...
                  <div key={opt.label} className="flex items-center gap-3">
                    <span className="text-sm text-slate-400 w-32 shrink-0">{opt.label}</span>
            ...
            {/* QR mock */}
            <div className="w-36 flex flex-col items-center justify-center gap-2 border-l border-slate-800 pl-6">
              <div className="w-20 h-20 rounded-xl bg-slate-800 grid grid-cols-5 gap-0.5 p-1.5">
                {Array.from({ length: 25 }).map((_, i) => (
                  <div
                    key={i}
                    className={`rounded-sm ${[0,1,2,3,4,5,9,10,14,15,19,20,21,22,23,24,6,12,18].includes(i) ? "bg-slate-400" : "bg-slate-800"}`}
                  />
                ))}
              </div>
              <span className="text-xs text-slate-500 text-center">Сканируйте<br/>для участия</span>
              <span className="font-mono text-white text-sm font-bold">ABC123</span>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-5xl px-8 pb-24">
        ...
        <p className="text-center text-slate-400 text-sm mb-10">Под любой сценарий — от обратной связи до командного планирования</p>
      ...
      <section className="mx-auto max-w-2xl px-8 pb-24 text-center">
        <div className="rounded-2xl border border-indigo-500/20 bg-indigo-500/5 p-12">
          ...
          <p className="text-slate-400 mb-8">Бесплатно. Без кредитной карты.</p>
          <Link
            href="/auth/register"
            className="inline-block rounded-xl bg-indigo-600 hover:bg-indigo-500 px-10 py-3.5 text-base font-semibold transition-colors"
          >
            Создать аккаунт
          </Link>
        </div>
      ...
      {/* Footer */}
      <footer className="border-t border-slate-800/50 px-8 py-6 text-center text-xs text-slate-600">
        © 2025 LivePoll AI
      </footer>
    </div>
  );
